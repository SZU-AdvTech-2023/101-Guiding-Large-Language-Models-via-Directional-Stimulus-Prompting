...
from rl4lms.envs.text_generation.test_reward import (RewardIncreasingNumbers,
                                                     ...
                                                     GPT3,LLAMARewardSummarizationWithHint)
...
class LLAMASummarizationWithHintMetric(BaseMetric):
    def __init__(self,
                 split_token: str,
                 split_token_id: int,
                 temperature: float,
        ...
        for gpt3_metric in gpt3_metrics:
            ...
            if gpt3_metric_type == 'rouge':
                gpt3_score_keys = ["lexical/rouge_rouge1", "lexical/rouge_rouge2", "lexical/rouge_rougeL",
                                   "lexical/rouge_rougeLsum"]
    ...
    def compute(self, prompt_texts: List[str],
                ...
                split_name: str = None,
                llm_tokenizer=None,
                llm_pipeline=None) -> Dict[str, float]:

        print("进入到compute了")
        # upper_gpt3_generated_texts, lower_gpt3_generated_texts = [], []
        llm_rewards = [[[] for _ in range(len(gpt3_score_keys))] for gpt3_score_keys in self.gpt3_score_keys]
        # lower_baseline_gpt3_rewards = [[[] for _ in range(len(gpt3_score_keys))] for gpt3_score_keys in
        #                                self.gpt3_score_keys]
        # upper_baseline_gpt3_rewards = [[[] for _ in range(len(gpt3_score_keys))] for gpt3_score_keys in
        #                                self.gpt3_score_keys]
        t5_rewards = [[[] for _ in range(len(t5_score_keys))] for t5_score_keys in self.t5_score_keys]  # 是个二维列表
        ...
        for i, t5_gen_text in enumerate(generated_texts):
            ...
            for j, t5_metric in enumerate(self.t5_metrics):
                t5_metric_type = self.t5_metric_types[j] # 1个值，hint_hit
                t5_score_keys = self.t5_score_keys[j] # 2个值，hint_hit和hint_num
            ...
            # llm generation
            if self.use_llm:
                print(len(generated_texts)) #5
                print("在compute这个循环里，i=",i)
                prompt_text = prompt_texts[i]
                t5_input_text = prompt_text

                llm_input_text = self.hint_prompt.replace("[[QUESTION]]", t5_input_text)
                llm_input_text = llm_input_text.replace("[[HINT]]", t5_gen_text)
                ...
                llm_gen_texts = [
                    LLAMARewardSummarizationWithHint.clean_generation(seq['generated_text'], self.stop_words) for
                    seq in sequences]
                llm_gen_texts = LLAMARewardSummarizationWithHint.generation_selection(self.selection_strategy,
                                                                                      llm_gen_texts)
                llm_generated_texts.append(*llm_gen_texts)
                # reward for llm
                for j, gpt3_metric in enumerate(self.gpt3_metrics):
                    ...
                    for g, llm_gen_text in enumerate(llm_gen_texts):
                        metric_results = gpt3_metric.compute([t5_input_text], [llm_gen_text], [reference_text])
                    ...
                    for k, score_key in enumerate(gpt3_score_keys):
                        ...
                        llm_rewards[j][k].append(avg_score)
        ...
        if self.evaluate_policy_model:
            for i, score_keys in enumerate(self.gpt3_score_keys):
                for j, score_key in enumerate(score_keys):
                    metric_dict[f"llm/{score_key}"] = (llm_rewards[i][j], np.mean(llm_rewards[i][j]))
            metric_dict["llm_generated_text"] = (llm_generated_texts, 0.)
