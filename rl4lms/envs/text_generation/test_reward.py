...
class LLAMARewardSummarizationWithHint(RewardFunction):
    def __init__(self,
                 temperature: float,
                 max_tokens: int,
                 num_seqs: int,
        ...
        # reward for gpt3: （照样使用gpt3的metrics）
        args = {}
        ...
        elif gpt3_metric == 'bert_score':
            self.gpt3_score_keys = ['semantic/bert_score']
        else:
    ...
    #     # print(candidates)
    #     # print(candidates[0])
    #     candidates = RewardSummarizationWithHint.clean_generation(candidates[0], stop_words + ["\n\n", "\n"])
    #     # exit()
        ...
        if done:
            ...
            if self.t5_coef != 0:
                if self.t5_metric_type == "rouge":
                    ...
                    metric_dict = self.t5_metric.compute(None, [t5_gen_text], [references])
                    t5_reward = metric_dict["keyword/hint_hit"][1] - self.t5_neg_coef * \
                                metric_dict["keyword/hint_not_hit"][1]
                else:
                    raise NotImplementedError
            ...
            if llm_tokenizer is not None:
                ...
                llm_gen_texts = [
                    LLAMARewardSummarizationWithHint.clean_generation(seq['generated_text'], self.stop_words) for seq in
                    sequences]
                llm_gen_texts = LLAMARewardSummarizationWithHint.generation_selection(self.selection_strategy,
                                                                                      llm_gen_texts)
                print("====================================")
                print("<LLM>", llm_gen_texts[0], "<LLM>")
                print("====================================")
            else:
                llm_gen_texts = [t5_gen_text]


            # reward for llm
            llm_rewards = []
            for i, llm_gen_text in enumerate(llm_gen_texts):
                metric_dict = self.gpt3_metric.compute(None, [llm_gen_text], [references])
                llm_reward = [metric_dict[k][1] for k in self.gpt3_score_keys]
                llm_reward = np.mean(llm_reward)
                llm_rewards.append(llm_reward)
            llm_reward = np.mean(llm_rewards)
            ...
            #         self.gpt3, gpt3_input_text, self.temperature, self.max_tokens, self.num_seqs, self.top_p,
            #         self.stop_words)
            #     gpt3_hint_gen_texts = RewardSummarizationWithHint.generation_selection(self.selection_strategy,
            #                                                                            gpt3_hint_gen_texts)
            #     # reward for gpt3
            #     gpt3_rewards = []
            ...
            reward = 10. * llm_reward + self.t5_coef * t5_reward
            print(f"llm: {llm_reward}, t5: {t5_reward}, total: {reward}")
            return reward
