...
class NLPO(OnPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        self.policy = self.policy.to(device='cuda:0')
        print(self.device)
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for batch_ix, rollout_data in enumerate(list(self.rollout_buffer.get(self.batch_size))):
                ...
                # 更改的内容
                r_dim = advantages.shape[-1]
                b_dim = advantages.shape[0]
                ratio = ratio.repeat(r_dim).reshape(r_dim,b_dim).transpose(0,-1)
                # 计算log pi的梯度
                logprob_grad = ratio.detach() * advantages.detach()
                # Trust Region 范围以外的为0
                logprob_grad[torch.where(ratio > 1 + clip_range)] = 0
                logprob_grad[torch.where(ratio < 1 - clip_range)] = 0
                grads_list = [-logprob_grad[:, i] for i in range(r_dim)]
                # 改到这里了 12.05

                # reference_vector 让多个目标侧重哪个目标
                # ref_vec是传参传进来的，此处需要看在哪里加个参数
                ref_vec = th.tensor(ref_vec, device=self.device, dtype=th.float32)
